...
public class CasDeTestControlleur {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<CasDeTest> getCasDeTest(@PathVariable(value="refCasTest") Long refCasTest) throws ResourceNotFoundException {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<CasDeTest> updateCasDeTest(@PathVariable (value = "refCasTest", required = false) Long refCasTest, @RequestBody CasDeTest casDeTestRequest ) throws ResourceNotFoundException {
        casDeTestRequest.setRefCasTest(refCasTest);
        return ResponseEntity.ok().body(casDeTestService.updateCasDeTest(casDeTestRequest));
    }

    @DeleteMapping("/{id}")
    public String deleteCasDeTest(@PathVariable(value="refCasTest") Long refCasTest){
