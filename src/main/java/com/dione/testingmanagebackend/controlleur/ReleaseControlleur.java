...
public class ReleaseControlleur {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Releas> getTesteur(@PathVariable(value="refRelease") Long refRelease) throws ResourceNotFoundException {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Releas> updateRelease(@PathVariable (value = "refRelease", required = false) Long refRelease, @RequestBody Releas releaseRequest ) throws ResourceNotFoundException {
        releaseRequest.setRefRelease(refRelease);
        return ResponseEntity.ok().body(releasService.updateReleas(releaseRequest));
    }

    @DeleteMapping("/{id}")
    public String deleteRelease(@PathVariable(value="refRelease") Long refRelease){
